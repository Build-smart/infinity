...
class AlertController extends Controller
{
	...
	public function forgotPasswordAlert($existUser){
		...
		// added to get the id....	
			$user = DB::table('users')->where('email', $existUser[0]->email)->first();

			//get device id
			$device_id = $this->userDevice($user->id);
			if(!empty($device_id)){
				$response = $this->$functionName($device_id, $sendData);
			}
		}
	...
	public function onesignalNotification($device_id, $sendData){
		...
		curl_setopt($ch, CURLOPT_HTTPHEADER, array('Content-Type: application/json; charset=utf-8',
				   'Authorization: Basic ZTBlNWI3MjAtN2UwNy00MDVjLTk3NDEtNDg3YTY1NDI4MWU4'));
		curl_setopt($ch, CURLOPT_RETURNTRANSFER, TRUE);
	...
	//orderAlert
	public function purchaseorderAlert($description,$title,$distributor_id){
		...
		//setting
		$setting = $this->setting();
		$ordersData['app_name'] = "Builder Mart Associate Partner";
		...
		if($alertSetting[0]->order_notification==1){
			...
			//get device id
			$device_id = $this->userDevice($distributor_id);
