...
/*************************************************************************/
        ...
        $limit=4;
// special type is added as this is taking long time....

$startTime=(microtime(true) - LARAVEL_START);
      ...
      /*  $newest_products = Cache::remember('newest_products', 120, function () {
    return $this->products->products($data);
       ...
       // $endTime=(microtime(true) - LARAVEL_START);
       // $actualTime=$endTime-$startTime;
        
        //  $result['actualTime_newest_products'] = $actualTime;
...
//is feature
       /* $data = array('page_number' => '0', 'type' => 'is_feature', 'limit' => $limit, 'min_price' => $min_price, 'max_price' => $max_price);
        $featured = $this->products->products*/ 
       // $result['featured'] = $featured;
/*
        $data = array('page_number' => '0', 'type' => '', 'limit' => '4', 'is_feature' => 1);
...
//current time
    ...
    public function contactus(Request $request)
    {
	    ...
	    //myContactUs
    public function constructions(Request $request)
    {
        $title = array('pageTitle' => Lang::get("website.Contact Us"));
        $result = array();
        $final_theme = $this->theme->theme();
        ...
        return view("web.constructions", ['title' => $title, 'final_theme' => $final_theme])->with('result', $result);
    }
 ...
 try {
    ...
    public function cost_calculator(Request $request)
    {
    	$title = array('pageTitle' => Lang::get("website.Contact Us"));
    	$result = array();
	 
	  	$selectedcity = $request->selectcity;
...
session(["SELECTEDAREA"=>$selectedarea]);
	 ...
	 //total cities
		...
		$allcities=DB::table('constructionresources')->distinct()->get(['city']);

		$result['allcities'] = $allcities;
	 
	 
		  if (empty(Session::get('SELECTEDCITY'))) {
          $cost_calculator= DB::table("constructionresources")->distinct()->where('city','=',$selectedcity)->get();
		$result['cost_calculator'] = $cost_calculator;
        }else{
				$cost_calculator= DB::table("constructionresources")->distinct()->get();
		$result['cost_calculator'] = $cost_calculator;
    	...
    	return view("web.cost_calculator", ['title' => $title, 'final_theme' => $final_theme])->with('result', $result);
    }
    ...
    public function setSession(Request $request){
      ...
      //mobile Cost Calculator Page
    public function mobile_cost_calculator(Request $request)
    {
    $title = array('pageTitle' => Lang::get("website.Contact Us"));
    $result = array();
    ...
    return view("web.mobile_cost_calculator", ['title' => $title, 'final_theme' => $final_theme])->with('result', $result);
    }
    


 //mobile constructions
    public function mobile_constructions(Request $request)
    {
    $title = array('pageTitle' => Lang::get("website.Contact Us"));
    $result = array();
    $final_theme = $this->theme->theme();
    ...
    return view("web.mobile_constructions", ['title' => $title, 'final_theme' => $final_theme])->with('result', $result);
    }
    
    
    //for webview for appp to show job postings 
    public function mobile_careers(Request $request)
    {
    $title = array('pageTitle' => 'Careers');
    $result = array();
    $final_theme = $this->theme->theme();
    ...
    return view("web.mobile_careers", ['title' => $title,'jobs' => $jobs, 'final_theme' => $final_theme])->with('result', $result);
    }


   public function mobile_aboutus(Request $request)
    {
    	$title = array('pageTitle' => 'About Us');
    	$result = array();
    	$final_theme = $this->theme->theme();
    	 $result['commonContent'] = $this->index->commonContent();
    
    	return view("web.mobile_aboutus", ['title' => $title, 'final_theme' => $final_theme])->with('result', $result);
    }
   ...
   public function aboutus(Request $request)
    {
    	...
    	return view("web.aboutus", ['title' => $title, 'final_theme' => $final_theme])->with('result', $result);
    }
    ...
    public function careers(Request $request)
    {
    	...
    	return view("web.careers", ['title' => $title,'jobs' => $jobs, 'final_theme' => $final_theme])->with('result', $result);
    }
    ...
    public function jobdetails(Request $request)
    {
    	...
    	$result = array();
    	$final_theme = $this->theme->theme();
    	$job_details =  $this->index->job_details($request->id);;
    	 
    	$result['commonContent'] = $this->index->commonContent();
    
    	return view("web.job_details", ['title' => $title,'job_details' => $job_details, 'final_theme' => $final_theme])->with('result', $result);
    }
    ...
    $time =  Carbon\Carbon::now();
    	...
    	if ($request->hasFile('resume') and in_array($request->resume->extension(), $extensions)) {
  	...
  	$toemail='onlineorders@buildermart.in';  	 
    	...
    	if (env('MAIL_USERNAME') && env('MAIL_PASSWORD')){
    		...
    		\Mail::send('/mail/appliedJobMail', array(
    			...
    			$message->to($toemail, 'Candidate')->subject('Applied For Job '.$job_name);
    		...
    		\Mail::send('/mail/candidateThankYouMail', array(), function($message) use ($candidate_email){
    			$message->from("onlineorders@buildermart.in");
    			$message->to($candidate_email, 'INFORMATION')->subject("BMLED & BUILDER MART INDIA PRIVATE LIMITED.");
    			//	 $message->attach(	$request->file('resume'));
    		});
