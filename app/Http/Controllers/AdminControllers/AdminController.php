...
class AdminController extends Controller
{
	...
	public function dashboard(Request $request){
		...
		$sale_amount = DB::table('orders')
		->LeftJoin('orders_status_history', 'orders_status_history.orders_id', '=', 'orders.orders_id')
		->LeftJoin('orders_status', 'orders_status.orders_status_id', '=', 'orders_status_history.orders_status_id')
		->LeftJoin('orders_status_description', 'orders_status_description.orders_status_id', '=', 'orders_status.orders_status_id')
		->where('orders_status_description.orders_status_name','=','Completed')	
	->where('orders_status_history.current_status','=','1')

		->sum('order_price');
		
	/*	$sale_tax_amount = DB::table('orders')
		->LeftJoin('orders_status_history', 'orders_status_history.orders_id', '=', 'orders.orders_id')
		->LeftJoin('orders_status', 'orders_status.orders_status_id', '=', 'orders_status_history.orders_status_id')
		->LeftJoin('orders_status_description', 'orders_status_description.orders_status_id', '=', 'orders_status.orders_status_id')
		->where('orders_status_description.orders_status_name','=','Completed')		
		...
		$purchase_total_sale_amount = DB::table('purchase_orders')
		...
		->where('purchase_orders_main_status.purchase_orders_main_status_name','=','Completed')
		->where('purchase_orders_status_history.current_status','=','1')
		->sum('roundup_cashdiscount_final_amount');
	 
		$result['purchase_total_sale_amount'] = $purchase_total_sale_amount;
  	...
  	/*foreach($products as $products_data){
  		...
  		$result['outOfStock'] =count($stock_0);
...
public function XXXlocationdashboard(Request $request){
		$title 			  = 	array('pageTitle' => Lang::get("labels.title_dashboard"));
		$language_id      = 	'1';
		...
		return view("admin.locationdashboard",$title)->with('result', $result);
	}
...
->where('orders.location_id', '=', $user_location_id)
->where('orders_status_description.orders_status_name','=','Completed')
// ->orderBy('orders_status_history.id','DESC')
...
return view("admin.locationdashboard",$title)->with('result', $result);
  ...
  public function checkLogin(Request $request){
		...
		if($validator->fails()){
			...
			$adminInfoSuperAdmin = array("email" => $request->email, "password" => $request->password,"role_id"=>1);

			$adminInfoAdmin = array("email" => $request->email, "password" => $request->password,"role_id"=>11);
			
			
			$adminInfoAdminManager = array("email" => $request->email, "password" => $request->password,"role_id"=>12);
						$adminInfoDataentry = array("email" => $request->email, "password" => $request->password,"role_id"=>13);


						$adminInfoPackDept = array("email" => $request->email, "password" => $request->password,"role_id"=>15);
           ...
           // if (Auth::attempt(['email' => $email, 'password' => $password, 'active' => 1])) {
			...
			if(auth()->attempt($adminInfoSuperAdmin)  || auth()->attempt($adminInfoAdmin)  || auth()->attempt($adminInfoAdminManager)  || auth()->attempt($adminInfoDataentry)  || auth()->attempt($adminInfoPackDept)  ) {
				$admin = auth()->user();
		...
		//	return redirect()->intended('admin/dashboard/this_month')->with('administrators', $administrators);
	 	...
	 	if($role_id==12){
	 	    ...
	 	    // manager 
				...
				return redirect()->intended('admin/locationdashboard/this_month')->with('administrators', $administrators);
				
			}else{
			    
			    //admin / super admin / data entry 
				return redirect()->intended('admin/dashboard/this_month')->with('administrators', $administrators);
   ...
   public function profile(Request $request){
	...
	public function addrole(Request $request){
		...
		if(count($roles)>0){
			...
			$locationdashboard_view = $roles[0]->locationdashboard_view;

	
			$orderdelivery_view = $roles[0]->orderdelivery_view;
			$orderdelivery_mapping = $roles[0]->orderdelivery_mapping;
			
	$locationorders_view = $roles[0]->locationorders_view;
			$locationreports_view = $roles[0]->locationreports_view;

			//Bharath 20-10-2020 Inventory Role Permissions
			$inventory_view = $roles[0]->inventory_view;
			$inventory_update = $roles[0]->inventory_update;
	...
	$locationdashboard_view =  '0';
	$orderdelivery_view = '0';
			$orderdelivery_mapping = '0';


			$locationorders_view = '0';
			$locationreports_view = '0';
		//Bharath 20-10-2020 Inventory Role Permissions
			$inventory_view = '0';
			$inventory_update = '0';
	...
	$result2[22]['link_name'] = 'Orderdelivery';
		$result2[22]['permissions'] = array(
				'0'=>array('name'=>'orderdelivery_view','value'=>$orderdelivery_view),
				'1'=>array('name'=>'orderdelivery_mapping','value'=>$orderdelivery_mapping),
		);


	//Bharath 20-10-2020 Inventory Permissions
		$result2[23]['link_name'] = 'Inventory';
		$result2[23]['permissions'] = array(
				'0'=>array('name'=>'inventory_view','value'=>$inventory_view),
				'1'=>array('name'=>'inventory_update','value'=>$inventory_update),
		);
		
		
				$result2[24]['link_name'] = 'Locationorders';
		$result2[24]['permissions'] = array(
				'0'=>array('name'=>'locationorders_view','value'=>$locationorders_view),
				 
		);



	$result2[25]['link_name'] = 'Locationreports';
		$result2[25]['permissions'] = array(
				'0'=>array('name'=>'locationreports_view','value'=>$locationreports_view),
					
		);
		
			$result2[26]['link_name'] = 'locationdashboard';
		$result2[26]['permissions'] = array('0'=>array('name'=>'locationdashboard_view','value'=>$locationdashboard_view));
	...
	public function addnewroles(Request $request){
		...
		$roles = DB::table('manage_role')->where('user_types_id',$request->user_types_id)->insert([
				...
				'locationdashboard_view'=>$request->locationdashboard_view,

	
				'orderdelivery_view' => $request->orderdelivery_view,
				'orderdelivery_mapping' => $request->orderdelivery_mapping,

							'locationorders_view' => $request->locationorders_view,
					'locationreports_view' => $request->locationreports_view,


				'inventory_view' => $request->inventory_view,
  ...
  //editCategoriesRoles
	...
	public function blockips(Request $request)
	{
		...
		$title = array('pageTitle' => "Block IPs List");
	
		$result = array(); 
		$block_ips = DB::table('block_ips')->paginate(50);
	
		$result['block_ips'] = $block_ips;
		$result['commonContent'] = $this->Setting->commonContent();
	
		return view("admin.ipblock.index", $title)->with('result', $result);
	}
	
	public function deleteips(Request $request)
	{
		DB::table('block_ips')->where('id', $request->id)->delete();
		return redirect()->back()->withErrors("Blocked Ip Deleted Successfully");
	}
	
	
	
 public function productsforalllocations(Request $request)
{
...
$date_added = date('Y-m-d h:i:s');
    ...
    $products = DB::table('products')
    ->leftJoin('products_description','products_description.products_id','=', 'products.products_id')
     ->select('products.*','products_description.language_id', 'products_description.products_name',
    'products_description.products_description', 'products_description.products_url', 'products_description.products_viewed',
    'products_description.products_left_banner', 'products_description.products_left_banner_start_date', 'products_description.products_left_banner_expire_date',
    'products_description.products_right_banner', 'products_description.products_right_banner_start_date', 'products_description.products_right_banner_expire_date')
   
    ->where('products.location_id',1)->skip($skip)->take($take)->get();
    ...
    'products_left_banner' => $product->products_left_banner,
    'products_left_banner_start_date' => $product->products_left_banner_start_date,
    'products_left_banner_expire_date' => $product->products_left_banner_expire_date,
    'products_right_banner' => $product->products_right_banner,
    'products_right_banner_start_date' => $product->products_right_banner_start_date,
    'products_right_banner_expire_date' => $product->products_right_banner_expire_date,
    'products_description' => $product->products_description
   
    ]);
   
   
    $special_details = DB::table('specials')->where('products_id',$product->products_id)->first();
    ...
    ->insert([
    'products_id' => $products_id,
    'specials_new_products_price' => $special_details->specials_new_products_price,
    'specials_date_added' => $special_details->specials_date_added,
    'expires_date' => $special_details->expires_date,
    'status' => $special_details->status,
    ]);
    }
   
   
    $products_to_categories = DB::table('products_to_categories')->where('products_id',$product->products_id)->get();
    foreach($products_to_categories as $prodcat) {
   
    DB::table('products_to_categories')
    ->insert([
    'products_id' => $products_id,
    'categories_id' => $prodcat->categories_id
    ]);
    }
...
$message = "Products Added for location  $location_id from $skip to $take  Successfully";
        return redirect()->back()->withErrors([$message]);
}
