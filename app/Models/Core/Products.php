...
class Products extends Model
{
	...
	public function paginator($request){
        ...
        $data = $this->sortable(['products_id'=>'DESC'])
            ...
            $data->leftJoin('products_to_categories', 'products.products_id', '=', 'products_to_categories.products_id')
                ...
                ->leftJoin('location', 'products.location_id', '=', 'location.id');
                


       /* $data->select('products.products_id as products_id,products.products_quantity as products_quantity,
        products.products_model as products_model, products.products_image as products_image,
        products.products_video_link as products_video_link,products.products_price as products_price,
        products.products_date_added  as products_date_added ,products.products_last_modified as products_last_modified,
        products.products_date_available as products_date_available,products.products_weight as products_weight,
        products.products_weight_unit as products_weight_unit,products.products_status as products_status,
        products.is_current as is_current,products.products_tax_class_id as products_tax_class_id,
        products.manufacturers_id as manufacturers_id,products.products_ordered as products_ordered,
        products.products_liked as products_liked,products.low_limit as low_limit,
        products.is_feature as is_feature,products.products_slug as products_slug,products.products_type as products_type,
        products.products_min_order as products_min_order,products.products_max_stock as products_max_stock, products.created_at as created_at,
        products.updated_at as updated_at', 'products_description.*', 'specials.specials_id', 'manufacturers.manufacturer_name','manufacturers.manufacturers_id',
        ...
        'specials.expires_date', 'image_categories.path as path', 'products.updated_at as productupdate', 'categories_description.categories_id as categories_id',
        'categories_description.categories_name','location.location_name')
        ...
        $data->select('products.*', 'products_description.products_name', 'specials.specials_id', 'manufacturers.manufacturer_name',
        'specials.products_id as special_products_id', 'specials.specials_new_products_price as specials_products_price',
        'specials.specials_date_added as specials_date_added', 'specials.specials_last_modified as specials_last_modified',
        'specials.expires_date', 'image_categories.path as path', 'products.updated_at as productupdate', 'categories_description.categories_id as categories_id',
        'categories_description.categories_name','location.location_name')
            ->where('products_description.language_id', '=', $language_id);
                ...
                if(empty($_REQUEST['location_id'])){
            	 ...
            	 $data->where('products.location_id', '=', 1);
        } else{
        	$data->where('products.location_id', '=', $_REQUEST['location_id']);
        }
               
        if(empty($_REQUEST['categories_id'])){
        	 
        	 $data->where('categories.categories_id','=',57);
        } else{
        	$data->where('categories.categories_id', '=', $_REQUEST['categories_id']);
        }   
        
            
            //   ->where('products.location_id', '=', 1)
          //     ->where('categories.categories_id','=',57);
          //  ->where('categories_description.language_id', '=', $language_id);
         ...
         /*   if (!empty(session('categories_id'))) {
          ...
          /*  $data->where('products_to_categories.categories_id', '=', $_REQUEST['categories_id']);*/
            ...
            if (isset($_REQUEST['product']) and !empty($_REQUEST['product'])) {
             $data->where('products_description.products_name', 'like', '%' . $_REQUEST['product'] . '%')
                ->orWhere('products.products_id','=', $_REQUEST['product']);
            }
    ...
    $products_id = DB::table('products')->insertGetId([
        ...
        'products_quantity' => 0,
        	'distributor_id'=>$request->distributor_id,
        	    		'distributor_product_price_percentage'=>$request->distributor_product_price_percentage,

    		'distributor_product_price'=>$request->distributor_product_price,
    	...
    	'product_segment' => $request->product_segment,
  ...
  public function edit($request){
    ...
    $result['units'] = $myVarsetting->getUnits();
       ...
       $result['locations'] = $myVarsetting->getLocations();
...
$distributors = DB::table('users')->where('customer_type','DISTRIBUTOR')->where('status','1')->get();
              ...
              $checkRecord = DB::table('products_attributes')->where([
                ...
                if(count($checkRecord)>0){
                    ...
                    $products_attributes_id = DB::table('products_attributes')->insertGetId([
                        ...
                        'distributor_options_values_price' => $request->distributor_options_values_price,
  ...
  public function currentstock($request){
    ...
    public function inactive_products($request){
    	$setting = new Setting();
    	$myVarsetting = new SiteSettingController($setting);
    	$commonsetting = $myVarsetting->commonsetting();
    	$myVaralter = new AlertController($setting);
