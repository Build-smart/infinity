...
class Customer extends Model
{
    ...
    public function updateMyProfile($request)
    {
        ...
        $gst = $request->gst;
        $pan = $request->pan;
        $customers_upi_address = $request->customers_upi_address;
        ...
        $customer_data = array(
        	...
        	'bank_name' => $bank_name,
        	'bank_ifs_code' => $bank_ifs_code,
        	'gst' => $gst,
        	'pan' => $pan, 
        'upi_address' => $customers_upi_address,
        );
    ...
    public function handleSocialLoginCallback($social)
    {
        ...
        if ($social == 'facebook') {
            ...
            $existUser = DB::table('users')
                ->Where('users.email', '=', $email)->where('role_id',2)->get();
   ...
   public function ExistUser($email)
    {
    	...
    	if(is_numeric($email)){
    		...
    		$existUser = DB::table('users')->where('role_id', 2)->where('phone', $email)->get();
    ...
    public function signupProcess($request)
    {
      ...
      //  $user_email = DB::table('users')->select('email','phone')->where('role_id', 2)->where('email', $email)->orwhere('phone', $phone)->get();
        $existUser = DB::table('users')->where('email', $email)->orWhere('phone', $phone)->first();
            ...
            if (DB::table('users')->insert([
              ...
              //  'gender' => $request->gender,
               ...
               // 'dob' => $customers_dob,
                ...
                if (auth()->guard('customer')->attempt(['email' => $request->email, 'password' => $request->password,'phone_verified'=>1])) {
                    $res['auth'] = "true";
                    $customer = auth()->guard('customer')->user();
                    //set session
                    session(['customers_id' => $customer->id]);
