...
class Cart extends Model
{
                   ...
                   /* if (count($currentStocks) > 0) {
                    ...
                    if (!empty($cart_data->max_order) and $cart_data->max_order != 0) {
                       /* if ($cart_data->max_order >= $stocks) {
                         ...
                         $result[$index]->max_order = $cart_data->max_order;
                    } else {
                        $result[$index]->max_order = $stocks;
                    }
                    $index++;
    ...
    public function deleteAllInCart()
    {
        ...
        $cart->delete();
		  ...
		  $cart_attributes = DB::table('customers_basket_attributes');

        if (empty(session('customers_id'))) {
            $cart_attributes->where('customers_basket_attributes.session_id', '=', Session::getId());
        } else {
            $cart_attributes->where('customers_basket_attributes.customers_id', '=', session('customers_id'));
        }
      

        $cart_attributes->delete();
    ...
    public function XXXaddToCartXXX($request)
    {
        $index = new Index();
        $products = new Products();
...
//change of price on the attribute change
 ...
 //Bharath  discounted price to change mrp and attribute price value
        ...
        if ($result['detail']['product_data'][0]->products_type == 1) {
            ...
            $values_price=0;
            $distributor_options_values_price=0;
           
                foreach ($attributeid as $attribute) {
                    $productattribute = DB::table('products_attributes')->where('products_attributes_id', $attribute)->first();
                    $symbol = $productattribute->price_prefix;
                   
                 
               
                    $values_price =  $productattribute->options_values_price;
                    $distributor_options_values_price =   $productattribute->distributor_options_values_price;
 ...
 //Bharath  discounted price to change mrp and attribute price value
         ...
         if (empty($customers_id))
            {
                ...
                $result =  Cart::addToCartByDeviceId($request);
            }
            else
            {
             
                $result = Cart::addToCartByCustomerId($request);
            }
    ...
    public function addToCartByCustomerId($request)
    {
      ...
      //  }
            $session_id ="";
       // $session_id = Session::getId();
            $customer_data =  DB::table('customers_basket')->where('customers_id','=',$customers_id)->where('is_order',0)->first();
            if($customer_data)
            {
        ...
        if ($result['detail']['product_data'][0]->products_type == 1) {
            ...
            $distributor_options_values_price=0;
                ...
                foreach ($attributeid as $attribute) {
                    ...
                    $product_price= $product_price + $values_price;
                      $distributor_product_original_price = $distributor_product_original_price +intval($distributor_options_values_price);
