...
class Index extends Model
{
    public function slides($currentDate)
    {
       /* $slides = DB::table('sliders_images')
            ...
            ->get();*/
              ...
              $slides = Cache::rememberForever('websliders', function () {
    ...
    public function commonContent()
    {
        ...
        if (empty(Session::get('language_id'))) {
		  ...
		  if (empty(Session::get('location_id'))) {
          
			
			
			 session(['location_id' =>$default_locations->id]); 
        
        		 session(['location_name' =>$default_locations->location_name]); 
...
//STORING THE CATEGOREIS IN THE SESSION FOR faster accessing....
  ...
  $result['categories']=session('CATEGORIES');
		} 
         */
		          ...
		          ->LeftJoin('categories_description', 'categories_description.categories_id', '=', 'promotional_banners.category_id')
		          ->select( 'promotional_banners.*', 'image_categories.path','categories_description.categories_name')
		         // ->where('categories_description.categories_name','=','Civil Supplies')
		          ->where('image_categories.image_type','ACTUAL')
		...
		//brand directories...
		  ...
		  $brandcategory = DB::table('manufacturers')
       ->LeftJoin('categories_description', 'categories_description.categories_id', '=', 'manufacturers.categories_id')
      ->select('manufacturers.*','categories_description.categories_name as name')
      ->where('categories_description.language_id', '=', 1)
       //->groupby('manufacturers.categories_id')
 		  ->get();
		//brand directories
		 
 		  
		 $result['brandcategory'] = $brandcategory;
...
private function categories()
    {
       ...
       /* $categories = DB::table('categories')
            ...
            //except BM Products all the categories will be loaded....
                       ->where('categories_description.categories_name', '!=', 'BM Products')
            ...
            $categories = Cache::rememberForever('webcategories', function () {
        ...
        foreach ($categories as $categories_data) {
          ...
          /*  $products = DB::table('categories')
            ...
            */
             ...
             $categories_data->total_products =0;
...
*/
 $sub_categories_data->total_products = 0;
//products_image
            $default_images = DB::table('image_categories')
                ->where('image_id', '=', $sub_categories_data->sub_image)
              // ->where('image_type', 'MEDIUM')
                ->where('image_type', 'ACTUAL')
                ->first();

            if ($default_images) {
                $sub_categories_data->path = $default_images->path;
            } else {
                $default_images = DB::table('image_categories')
                    ->where('image_id', '=', $sub_categories_data->sub_image)
                   // ->where('image_type', 'MEDIUM')
    ...
    private function XXXcategories()
    {
    ...
    private function specialCategories($parentId)
    {
    	...
    	->select('categories.categories_id as id',
    			...
    			->where('categories.categories_status', 1)
    			//->where('categories.categories_slug',$categorySlug)
    			->groupBy('categories.categories_id')
    			->get();
      ...
      public function job_details($id)
    {
    	...
    	$job_details = DB::table('jobs')->where('id', $id)->first();;
    	return $job_details;
    }
