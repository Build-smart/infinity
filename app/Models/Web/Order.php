...
class Order extends Model
{
    ...
    public function place_order($request)
    {
        ...
        $result['cart'] = $cart_items;
              ...
              // To Bring Location Id From Products of Orders
      /*  $product_id= $result['cart'][0]->products_id;
      ...
      $product_details = DB::table('products')
        ...
        $delivery_alternate_phone = session('shipping_address')->delivery_alternate_phone;
        ...
        $billing_alternate_phone = session('billing_address')->billing_alternate_phone;
        ...
        $order_distributor_price = session('distributor_price');
        ...
        if ($payment_method == 'braintree') {
            ...
            $payments_setting = $this->payments_setting_for_buildermart_wallet();
            ...
            $payment_status = 'success';
             ...
             $order_informationx = session('paymentResponseData');
             
             
            					$order_information = array('payment_id'=>$request->paymentID, 'transaction_id'=>$request->nonce,'order_payment_id'=>$request->order_payment_id);         
//$order_information=json_encode($order_information)." session reponse ".json_encode($order_informationx);
          /*
        ...
        if ($payment_status == 'success') {            
            ...
            $orders_id = DB::table('orders')->insertGetId(
                ['customers_id' => $customers_id,
                    ...
                    'cashback_amount' => 0,
                    'delivery_alternate_phone' => $delivery_alternate_phone,
            ...
            foreach ($order as $data) {
              ...
              /* $orders_products = DB::table('orders_products')
                ...
                foreach ($orders_products as $orders_products_data) {
                    ...
                    $total_tax     +=$orders_products_data->products_tax;
            ...
            //this is commented on 31 jan 2022 , as thisshould be in the on order completion ....
...
if(auth()->guard('customer')->user()->customer_type=="BUSINESSOWNER"){
            DB::table('orders')
...
if(auth()->guard('customer')->user()->customer_type=="CUSTOMER" || auth()->guard('customer')->user()->customer_type=="BUSINESSOWNER"){
    ...
    public function orders($request)
    {
       ...
       /* if (Session::get('guest_checkout') == 1) {
            ...
            $orders = DB::table('orders')->orderBy('date_purchased', 'DESC')->where('customers_id', '=', auth()->guard('customer')->user()->id)->get();
             ...
             //orders
        if (Session::get('guest_checkout') == 1) {
            $orders = DB::table('orders')->orderBy('date_purchased', 'DESC')->where('customers_id', '=', Session::get('customers_id'))
             ->get();
        } else {
  ...
  ->select('image_categories.path as image', 'products.products_model as model', 'orders_products.*','product_orders_status.product_orders_status_name')
                    ->where('orders_products.orders_id', $orders_data->orders_id)
 ->get();
    ...
    public function calculateTax($tax_zone_id)
    {
        ...
        if ($tax_zone_id == -1) {
            ...
            foreach ($cart as $products_data) {
                ...
                if (count($products) > 0) {
                    $tax_value = $products[0]->tax_rate / 100 * $final_price * $products_data->customers_basket_quantity;
                    $total_tax +=  $tax_value;
                 //  echo $index;
                    $index++;
...
//echo "cal".$tax;
    ...
    public function payments_setting_for_buildermart_wallet()
    {
