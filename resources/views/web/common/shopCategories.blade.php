<?php
 ...
 function shopChildcat($childs, $i, $parent_id){
  ...
  foreach($childs as $key => $child){
    ...
    $contents.= '<li class="list-item">
        <a class="list-link" href='.url('shop?category=').$child->slug.' > 
   ...
   //   $contents.= shopChildcat($child->childs,$k,$parent_id);
        ...
        $contents.= ' <ul class="unorder-list" style="margin-left:15px;">';
      $contents.= shopChildcat($child->childs,$k,$parent_id);
      $contents.= ' </ul>';
    }
    elseif($i>0){
 ...
 function shopRecursivecategories(){
  ...
  $items = Cache::rememberForever('leftsidebar_web_allcategories'  ,function ()  { 
