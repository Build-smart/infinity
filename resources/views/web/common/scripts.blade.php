...
function getQuantity(isCart){
  ...
  jQuery(document).on('click', '.qtyplus', function(e){
    ...
    var maximumVal =  parseInt(jQuery('#maxqty').val());
...
jQuery(document).ready(function() {
  ...
  //{{ $functionValue }}();
	...
	jQuery(document).on('change', '.{{ $functionValue }}', function(e){
		    ...
		    //is the attribute value is null 11-2-2022
		        var current_product_price = jQuery('#products_price').val();
		    
		    var original_price = jQuery('#original_price').val();
  			var discounted_percent = jQuery('#discounte_percent').val();
		  ...
		  //is the attribute value is null 11-2-2022
		  var total_price = current_product_price;
		     if(value_price !=0 ){
		    if(prefix.trim() == '+' ){

		    	 var current_original_price=parseFloat(original_price) + parseFloat(value_price);
		    	 
		      var discount_price =parseFloat(current_original_price) - parseFloat(current_original_price) * discounted_percent/100;
		    ...
		    var total_price = products_price * <?=session('currency_value')?>;
		    jQuery('.total_price').html('<?=Session::get('symbol_left')?>'+total_price.toFixed(2)+'<?=Session::get('symbol_right')?>');
		    
		     jQuery('.original_price').html('<?=Session::get('symbol_left')?>'+current_original_price.toFixed(2)+'<?=Session::get('symbol_right')?>');
...
//productTitle=productTitle + '' + jQuery('option:selected', this).text();
	//     jQuery('#product_title').html('('+productTitle+')');

  
	  //bharath
	     var str = jQuery('.pro-title').text();
         productTitle=productTitle + '  ' + jQuery('option:selected', this).text();
         product_name = str.replace(/ \([\s\S]*?\)/g,' ('+productTitle+')');
	      // jQuery('#product_title').html('('+productTitle+')');
          jQuery('.pro-title').text(product_name);
	  
	     //bharath
		          jQuery('#product_selected_name').val(product_name);
...
jQuery(document).on('change', '.currentstock', function(e){
...
jQuery('.qtypluscart').click(function(e){
		...
		// Get its current value
    var currentVal = parseInt(jQuery(this).parents('span').prev('.qty').val());   
		var maximumVal =  jQuery(this).parents('span').prev('.qty').attr('maxorder');
	//	maxqty
...
//bharath get Cities start 
function getCities() {
	jQuery(function ($) {
		jQuery('#loader').show();
		var city_id = jQuery('#entry_zone_id').val();
		...
		jQuery.ajax({
			...
			success: function (res) {
				...
				for (i = 0; i < res.length; ++i) {
					var j = i + 1;
					showData[i] = "<option value='"+res[i].location_name+"'>"+res[i].location_name+"</option>";
				}
			//	showData.push("<option value='-1'>@lang('website.Other')</option>");
				jQuery("#city").html(showData);
				jQuery('#loader').hide();
			},
		});
	});
...
function getBillingCities(){
	jQuery('#loader').show();
 
	var city_id = jQuery('#billing_zone_id').val();
	jQuery.ajax({
		headers: {'X-CSRF-TOKEN': jQuery('meta[name="csrf-token"]').attr('content')},
		url: '{{ URL::to("/ajaxcities")}}',
		type: "POST",
		...
		success: function (res) {
			...
			jQuery("#billing_city").html(showData);
			jQuery('#loader').hide();
...
function getQuantity(isCart){
  ...
  var original_price = jQuery('#original_price').val();
	...
	//in quick view amount changes purpose
		 var prefix = jQuery('option:selected', this).attr('prefix');
		    ...
		    if(prefix.trim() == '+' ){
		    	   ...
		    	   original_price=parseFloat(original_price) + parseFloat(value_price);
		    	 
		      var discount_price =parseFloat(original_price) - parseFloat(original_price) * discounted_percent/100;
		    ...
		    if(isCart==0){
			 ...
			 jQuery('.total_price').html('<?=Session::get('symbol_left')?>'+total_price.toFixed(2)+'<?=Session::get('symbol_right')?>');
					     ...
					     jQuery('.original_price').html('<?=Session::get('symbol_left')?>'+original_price.toFixed(2)+'<?=Session::get('symbol_right')?>');
...
jQuery(document).on('click','#resendotp', function(e){


$("#resendotp").hide();

var formData = jQuery('#verifyotpform').serialize();

jQuery.ajax({
headers: {'X-CSRF-TOKEN': jQuery('meta[name="csrf-token"]').attr('content')},
url: '{{ URL::to("/resendLoginVerificationOTP")}}',
type: "post",
data: formData,
success: function (res) {
//alert(res);
           ...
           if(seconds == 0 )
{
    ...
    $("#resendotp").show();
           ...
           if(seconds>0){
            ...
            var msg="You can request for new OTP in: ";
            $('#timer').html(msg+result);
