        ...
        <div class="row">
            <div class="col-12 col-md-12">
              ...
              @endif
                ...
                <h5 class="pro-title">{{$result['detail']['product_data'][0]->products_name}}  <b id="product_title"></b></h5>
          ...
          <div class="price">                     
            ...
            <input type="hidden" id="original_price" value="{{$orignal_price}}">
            <input type="hidden"  id="discounte_percent"  value="{{(int)$discount_percentage}}">
...
@if(!empty($result['cart']))
            ...
            <price class="total_price">{{Session::get('symbol_left')}}{{$result['cart'][0]->final_price}}{{Session::get('symbol_right')}}</price>
            <span class="original_price">{{Session::get('symbol_left')}}{{$orignal_price+0}}{{Session::get('symbol_right')}} </span> 
            @else
...
<div class="price" style="font-size:12px;margin-top:10px">
     + GST T&C Apply  
     </div>  
...
<!--                 @endif -->
          ...
          <form name="attributes" id="add-Product-form" method="post" >
            <input type="hidden" name="products_id" value="{{$result['detail']['product_data'][0]->products_id}}">
             ...
             <input type="hidden" name="products_price" id="products_price" value="
            @if(!empty($result['cart'])) 
             {{$result['cart'][0]->final_price}}
            
            @else 
            @if(!empty($result['detail']['product_data'][0]->flash_price)) {{$result['detail']['product_data'][0]->flash_price+0}} @elseif(!empty($result['detail']['product_data'][0]->discount_price)){{$result['detail']['product_data'][0]->discount_price+0}}@else{{$result['detail']['product_data'][0]->products_price+0}}@endif
             @endif">
            ...
            <input type="hidden" id="max_order" value="@if(!empty($result['detail']['product_data'][0]->products_max_stock)) {{ $result['detail']['product_data'][0]->products_max_stock }} @else 0 @endif" >
                        ...
                        <input type="hidden" class="form-control" name="product_selected_name" id="product_selected_name" >
             ...
             ?>
               ...
               <input id="attributeids_<?=$index?>" type="hidden" name="attributeid[]" value="" >
                ...
                <div class="attributes col-12 col-lg-9 box">  
                     <label class="col-6">{{ $attributes_data['option']['name'] }}</label>
                     <div class="select-control">
                     <select name="{{ $attributes_data['option']['id'] }}" onChange="getQuantity(0)" class="currentstock form-control attributeid_<?=$index++?>" attributeid = "{{ $attributes_data['option']['id'] }}">
                       @if(!empty($result['cart']))
                         @php
                           $value_ids = array();
...
<!--      </li> -->
   ...
   <div class="content text-center">
     ...
     + GST T&C Apply  
      ...
      <img src="{{asset('images/bmassured.png')}}" width="150" style="margin-left:60px;" alt="BM Assured"> 
    @endif
