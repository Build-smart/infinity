...
$("img").click(function() {
  ...
  var customer_type =  jQuery('.customer_type').val();
 	          ...
 	          if(customer_type=="BUSINESSOWNER"){
 	        	  jQuery('#workforce').show();
 	        	  jQuery("#workforce_name").addClass("field-validate");

 	          }else{
 	        	  jQuery('#workforce').hide();
 	        	jQuery("#workforce_name").removeClass("field-validate");

 	          }    

 

  jQuery(document).on('change', '.customer_type', function(e){
		...
		success: function (res) {
			...
			if(res.length != ''){
				...
				//	showData[i] = "<tr><td>"+j+"</td><td>"+res[i].products_options_name+"</td><td>"+res[i].products_options_values_name+"</td><td>"+res[i].price_prefix+" "+res[i].options_values_price+"</td><td>    <a class='badge bg-light-blue editproductattributemodal' products_attributes_id = '"+res[i].products_attributes_id+"' products_id = '"+res[i].products_id+"'  language_id = '"+res[i].language_id+"' options_id= '"+res[i].options_id+"'><i class='fa fa-pencil-square-o' aria-hidden='true'></i></a> <a class='badge bg-red deleteproductattributemodal' products_attributes_id = '"+res[i].products_attributes_id+"' products_id = '"+res[i].products_id+"' ><i class='fa fa-trash' aria-hidden='true'></i></a></td></tr>";
					showData[i] = "<tr><td>"+j+"</td><td>"+res[i].products_options_name+"</td><td>"+res[i].products_options_values_name+"</td><td>"+res[i].price_prefix+" "+res[i].options_values_price+"</td><td>"+res[i].price_prefix+" "+res[i].distributor_options_values_price+"</td><td>    <a class='badge bg-light-blue editproductattributemodal' products_attributes_id = '"+res[i].products_attributes_id+"' products_id = '"+res[i].products_id+"'  language_id = '"+res[i].language_id+"' options_id= '"+res[i].options_id+"'><i class='fa fa-pencil-square-o' aria-hidden='true'></i></a> <a class='badge bg-red deleteproductattributemodal' products_attributes_id = '"+res[i].products_attributes_id+"' products_id = '"+res[i].products_id+"' ><i class='fa fa-trash' aria-hidden='true'></i></a></td></tr>";
...
$(document).ready(function () {
	        ...
	        $(document).on('keyup', ".special_price_percentage", function () {
	           ...
	           let specialprice =$("#product_"+data_id).val() - $("#product_"+data_id).val() * $("#specialproductpricepercentage_"+data_id).val()/100;
	           $("#specialproduct_"+data_id).val(specialprice.toFixed(2))
...
function changeNow(data_id){
	   $('#changepriceModal').modal({
         backdrop: 'static',
         keyboard: false
     })
     .on('click', '#changepriceNow', function(e) {
     	 
          var productprice=$("#product_"+data_id).val();
         
 jQuery.ajax({
   		    beforeSend: function (xhr) { // Add this line
   		            xhr.setRequestHeader('X-CSRF-Token', $('[name="_csrfToken"]').val());
   		     },
   		    url: '{{ url('admin/products/productpriceupdate') }}',
   		    type: "POST",
   		    data: {"data_id":data_id,"productprice":productprice,"_token": "{{ csrf_token() }}"},
   		    success: function (data) {
   		     ...
   		     if(data.status=='SUCCESS'){
   		      ...
   		      $("#message_"+data_id).html("Product Price Updated Successfully");
   		      $("#message_"+data_id).fadeOut(2000);
   		       window.location.reload();
   		      }else{
   		       $('#changepriceModal').modal('hide');
   		      	 $("#message_"+data_id).text("Product Price Updated Failed");
   		      	 $("#message_"+data_id).fadeOut(2000);
   		      window.location.reload();
   		      }
   		    },
     ...
     $("#cancel").on('click',function(e){
      e.preventDefault();
      $('#changepriceModal').modal.model('hide');
     });
...
$('.changespecialprice').on('click', function(e) {

	
	 //get the selected item. data id and pass to the  change_location
			 var data_id= $(this).attr('data-id');
			   
   specialchangeNow(data_id);
   });


function specialchangeNow(data_id){
	   $('#changespecialpriceModal').modal({
         backdrop: 'static',
         keyboard: false
     })
     .on('click', '#changespecialpriceNow', function(e) {
     	 
    	 var productspecialprice=$("#specialproduct_"+data_id).val()
         
 jQuery.ajax({
   		    beforeSend: function (xhr) { // Add this line
   		            xhr.setRequestHeader('X-CSRF-Token', $('[name="_csrfToken"]').val());
   		     },
   		    url: '{{ url('admin/products/productspecialpriceupdate') }}',
   		    type: "POST",
   		    data: {"data_id":data_id,"productspecialprice":productspecialprice,"_token": "{{ csrf_token() }}"},
   		    success: function (data) {
 
   		   
   		 
   		     if(data.status=='SUCCESS'){
   		      $('#changespecialpriceModal').modal('hide');
   		      $("#specialmessage_"+data_id).html("Product Special Price Updated Successfully");
   		      $("#specialmessage_"+data_id).fadeOut(2000);
   		   window.location.reload();
   		      }else{
   		       $('#changespecialpriceModal').modal('hide');
   		      	 $("#specialmessage_"+data_id).text("Product Special Price Update Failed");
   		      	 $("#specialmessage_"+data_id).fadeOut(2000);
   		      window.location.reload();
   		      }
   		    },
...
$('.addtowallet').on('click', function(e) {


//get the selected item. data id and pass to the  change_location
var data_id= $(this).attr('data-id');
 
addtowalletNow(data_id);
   });


function addtowalletNow(data_id){
  $('#addtowalletModal').modal({
         backdrop: 'static',
         keyboard: false
     })
     .on('click', '#addtowalletNow', function(e) {
     
  var added_wallet_date = $("#added_wallet_date_"+data_id).val();
  var purchase_order_id = $("#purchase_order_id_"+data_id).val();
 ...
 jQuery.ajax({
       ...
       url: '{{ url('admin/products/addwalletdate') }}',
       type: "POST",
       data: {data_id:data_id,purchase_order_id:purchase_order_id,
        added_wallet_date:added_wallet_date,"_token": "{{ csrf_token() }}"},
       success: function (data) {
        if(data.status=='SUCCESS'){
        $('#addtowalletModal').modal('hide');
         $("#message_"+data_id).html("Date Added Successfully");
         $("#message_"+data_id).fadeOut(2000);
      window.location.reload();
         }else{
        $('#addtowalletModal').modal('hide');
          $("#message_"+data_id).text("Date Added Failed");
          $("#message_"+data_id).fadeOut(2000);
         window.location.reload();
         }
       },
...
// Discount Accepted Or Rejected By Admin
$('.discount').on('click', function(e) {


//get the selected item. data id and pass  
var data_id= $(this).attr('data-id');
 discountacceptreject(data_id);
   });


function discountacceptreject(data_id){
  $('#discountModal').modal({
         backdrop: 'static',
         keyboard: false
     })
     .on('click', '#discountacceptreject', function(e) {
     
  var is_discount = $("#is_discount_"+data_id).val();
  var purchase_order_id = $("#purchase_orders_id_"+data_id).val();
       
 jQuery.ajax({
       beforeSend: function (xhr) { // Add this line
               xhr.setRequestHeader('X-CSRF-Token', $('[name="_csrfToken"]').val());
        },
       url: '{{ url('admin/products/discountacceptreject') }}',
       type: "POST",
       data: {data_id:data_id,purchase_order_id:purchase_order_id,
      is_discount:is_discount,"_token": "{{ csrf_token() }}"},
       success: function (data) {
        if(data.status=='SUCCESS'){
        $('#discountModal').modal('hide');
         $("#disocuntmsg_"+data_id).html(data.content);
         $("#disocuntmsg_"+data_id).fadeOut(2000);
      window.location.reload();
         }else{
        $('#discountModal').modal('hide');
          $("#disocuntmsg_"+data_id).text(data.content);
          $("#disocuntmsg_"+data_id).fadeOut(2000);
         window.location.reload();
         }
       },
...
//      url:"{{ url('admin/products/productpriceupdate') }}",
//      method:'POST',
//      data:{data_id:data_id,productprice:productprice},
//      dataType:'json',
//      success:function(data)
//      {
//     if(data.status=='SUCCESS'){
...
// $(document).on('click', ".changespecialprice", function () {
// 	  var data_id= $(this).attr('data-id');
...
//    url:"{{ url('admin/products/productspecialpriceupdate') }}",
//    method:'POST',
//    data:{data_id:data_id,productspecialprice:productspecialprice},
//    dataType:'json',
//    success:function(data)
//    {
//   if(data.status=='SUCCESS'){
...
{{--{{ URL::to('admin/deleteproductattributemodal')}}--}}
	...
	$(document).on('click', '#deleteAdminUserFrom', function(){
		var customers_id = $(this).attr('customers_id');
		$('#customers_id').val(customers_id);
...
//deleteDistributorModal
    $(document).on('click', '#deleteDistributorId', function(){
	...
	//deleteRewardPointModal
	$(document).on('click', '#deleteRewardPointId', function(){
		var reward_id = $(this).attr('reward_id');
		$('#reward_id').val(reward_id);
		$("#deleteRewardPointModal").modal('show');
	});

	//deleteClientModal
	$(document).on('click', '#deleteClientId', function(){
	...
	//deleteJobModal
	$(document).on('click', '#deleteJobId', function(){
	...
	//deleteIpModal
	$(document).on('click', '#deleteIPId', function(){
	...
	//deletePromotionalBannerModal
	$(document).on('click', '#deletePromotionalBannerId', function(){
		var promotionalbanner_id = $(this).attr('promotionalbanner_id');
		$('#promotionalbanner_id').val(promotionalbanner_id);
		$("#deletePromotionalBannerModal").modal('show');
	});

	//deleteWalletModal
	$(document).on('click', '#deleteWalletId', function(){
	...
	//deleteLocationModal
	$(document).on('click', '#deleteLocationId', function(){
		var location_id = $(this).attr('location_id');
		$('#location_id').val(location_id);
		$("#deleteLocationModal").modal('show');
	});

	//deletePurchaseorderstatusModal
	$(document).on('click', '#deletePurchaseorderstatusId', function(){
		var purchaseorderstatus_id = $(this).attr('purchaseorderstatus_id');
		$('#purchaseorderstatus_id').val(purchaseorderstatus_id);
		$("#deletePurchaseorderstatusModal").modal('show');
	});



	//deleteProductorderstatusModal
	$(document).on('click', '#deleteProductorderstatusId', function(){
		var productorderstatus_id = $(this).attr('productorderstatus_id');
		$('#productorderstatus_id').val(productorderstatus_id);
		$("#deleteProductorderstatusModal").modal('show');
	});


	//deletePurchaseordermainstatusModal
	$(document).on('click', '#deletePurchaseordermainstatusId', function(){
		var purchaseordermainstatus_id = $(this).attr('purchaseordermainstatus_id');
		$('#purchaseordermainstatus_id').val(purchaseordermainstatus_id);
		$("#deletePurchaseordermainstatusModal").modal('show');
	});


	$(document).on('click', '#deleteDistributorBannerId', function(){
		var distributorbanners_id = $(this).attr('distributorbanners_id');
		$('#distributorbanners_id').val(distributorbanners_id);
		$("#deleteDistributorBannerModal").modal('show');
	});

	//deleteConstructionResourceModal
	$(document).on('click', '#deleteConstructionResourceId', function(){
		var constructionresource_id = $(this).attr('constructionresource_id');
		$('#constructionresource_id').val(constructionresource_id);
		$("#deleteConstructionResourceModal").modal('show');
	});
	...
	//projectDocsModal
	$(document).on('click', '#ProjectDocsId', function(){
		var project_id = $(this).attr('project_id');
  
		$('#projects_id').val(project_id);
		$("#projectDocsModal").modal('show');
	});


	//projectFollowupsModal
	$(document).on('click', '#ProjectFollowupsId', function(){
		var project_id = $(this).attr('project_id');
   		$('#projectss_id').val(project_id);
		$("#projectFollowupsModal").modal('show');
	});
	$(".adddocbutton").hide();
	...
	$(document).on('click', '.followupdocument', function(){
...
$( "#walletcustomer" ).on('click','#submit',function( event ) {

	  var param =  $( "#customer_id" ).val();
...
// Bharath Added For Load Images for edit and add for new location
$(document).on('click','.product_image_close', function(){
...
//Bharath Added For Load Images for edit and add for Imagetwo
$(document).on('click','.selected_image_close', function(){
...
var id='';
 ...
 {
  $.ajax({
   url:'{{ URL::to("admin/products/loadmore/load_data")}}',
   method:"POST",
 ...
 $(document).ready(function () {
	 ...
	 $(document).on('click', '.selectimage', function(){
		  ...
		  $("#selectedthumbnail").html("<img height='90px' width='90px' src="+$("#"+id).attr("src")+"/>");
		  $('#selectedthumbnail').show();
			$('#image-close').show();
			$('#imageselected').removeClass('has-error');
			$('#newImage').removeClass('field-validate');
 ...
 $(document).ready(function () {
	 ...
	 $(document).on('click', '#compeleteimage', function(){
...
$(document).ready(function(){
 ...
 var _token = $('input[name="_token"]').val();
var id='';
 image_load_data(id, _token);

 function image_load_data(id="", _token)
 {
  $.ajax({
   url:'{{ URL::to("admin/products/imageloadmore/image_load_data")}}',
   method:"POST",
   data:{id:id, _token:_token},
   success:function(data)
   {
	   
    $('#image_load_more_button').remove();
    $('#image_post_data').append(data);
   
   }
  })
 }

 $(document).on('click', '#image_load_more_button', function(){

	  
  var id = $(this).data('id');
  $('#image_load_more_button').html('<b>Loading...</b>');
  image_load_data(id, _token);
 });


 
 $(document).on('click', '#imagerefresh', function(){
	 $('#image_post_data').html("");
 ...
 $(document).ready(function () {
	 ...
	 $(document).on('click', '.selectimagetwo', function(){

		  
		  var id = $(this).attr('id');


		  $(".selectimagetwo").removeAttr("style");
		  ...
		  $("#selectedthumbnailIcon").html("<img height='90px' width='90px' src="+$("#"+id).attr("src")+"/>");
		  $('#selectedthumbnailIcon').show();
			$('#image-Icone').show();
			$('#imageselectedtwo').removeClass('has-error');
			$('#newIcon').removeClass('field-validate');
			$('.thumbnail.selected').removeClass('selected');
			$('#selectedimage_id').val(id);
...
$(document).ready(function(){
 ...
 $('#products_search').keyup(function(){ 
       ...
       // alert(query);
        ...
        var _token = $('input[name="_token"]').val();
         $.ajax({
          url:"{{ URL::to("admin/products/inventory/getinventoryproducts")}}",
          method:"post",
    ...
    $(document).on('click', '.products_data', function(){  
          var data_id= $(this).attr('id');
            
        alert('You have selected '+$(this).attr('name')+' - '+$(this).attr('segment')+' - '+$(this).attr('location'));
       
          $('.product_name').val($(this).attr('name')+' - '+$(this).attr('segment')+' - '+$(this).attr('location'));
     
         $('#products_data').hide();
        $('#selected_products_id').val(data_id);  
        $('#productList').fadeOut();  
    });  
...
$(document).ready(function(){
 ...
 $('#products_search_coupon').keyup(function(){ 
       ...
       // alert(query);
        ...
        var _token = $('input[name="_token"]').val();
         $.ajax({
          url:"{{ URL::to("admin/coupons/getcouponproducts")}}",
          method:"post",
          data:{search:query, _token:_token},
          success:function(data){
             // alert(data)
    ...
    $(document).on('click', '.couponproducts_data', function(){  
          var data_id= $(this).attr('id');
            
     
          $('#selected_products_id').append($("<option value="+ data_id +">" + $(this).attr('name') + "</option>"));
...
$(document).ready(function(){
 ...
 $('#exclude_products_search_coupon').keyup(function(){ 
       ...
       // alert(query);
        ...
        var _token = $('input[name="_token"]').val();
         $.ajax({
          url:"{{ URL::to("admin/coupons/getcouponexcludeproducts")}}",
          method:"post",
          data:{search:query, _token:_token},
          success:function(data){
             // alert(data)
           $('#excludeproductList').fadeIn();  
           if(data=='PRODUCT NOT FOUND'){
                
           }
                    $('#excludeproductList').html(data);
          }
         });
        }
    });

    $(document).on('click', '.coupon_exclude_products_data', function(){  
          var data_id= $(this).attr('id');
            
     
          $('#selected_exclude_products_id').append($("<option value="+ data_id +">" + $(this).attr('name') + "</option>"));


          //  $('#products_data').hide();
 
        $('#excludeproductList').fadeOut();  
    });  
